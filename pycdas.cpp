...
static const char* flag_names[] = { "CO_OPTIMIZED", "CO_NEWLOCALS",
    "CO_VARARGS", "CO_VARKEYWORDS", "CO_NESTED", "CO_GENERATOR", "CO_NOFREE",
    "CO_COROUTINE", "CO_ITERABLE_COROUTINE", "CO_ASYNC_GENERATOR", "<0x400>",
    "<0x800>", "CO_GENERATOR_ALLOWED", "<0x2000>", "<0x4000>", "<0x8000>",
    "<0x10000>", "CO_FUTURE_DIVISION", "CO_FUTURE_ABSOLUTE_IMPORT",
    "CO_FUTURE_WITH_STATEMENT", "CO_FUTURE_PRINT_FUNCTION",
    "CO_FUTURE_UNICODE_LITERALS", "CO_FUTURE_BARRY_AS_BDFL",
    "CO_FUTURE_GENERATOR_STOP", "CO_FUTURE_ANNOTATIONS",
    "CO_NO_MONITORING_EVENTS", "<0x4000000>", "<0x8000000>", "<0x10000000>",
...
static void ivprintf(
    std::ostream& pyc_output, int indent, const char* fmt, va_list varargs)
{
    for (int i = 0; i < indent; i++)
        pyc_output << "    ";
    ...
    unsigned flags, std::ostream& pyc_output)
{
    ...
    case PycObject::TYPE_CODE2:
        {
            ...
            iprintf(
                pyc_output, indent + 1, "Arg Count: %d\n", codeObj->argCount());
            ...
            if (mod->verCompare(1, 3) >= 0) {
                ...
                if (mod->verCompare(3, 8) < 0) {
                    ...
                    orig_flags = (orig_flags & 0xFFFF) |
                                 ((orig_flags & 0xFFF00000) >> 4);
                }
            ...
            if (mod->verCompare(1, 3) >= 0) {
                ...
                for (int i = 0; i < codeObj->localNames()->size(); i++)
                    output_object(codeObj->localNames()->get(i), mod,
                        indent + 2, flags, pyc_output);
            }

            if (mod->verCompare(3, 11) >= 0 &&
                (flags & Pyc::DISASM_PYCODE_VERBOSE) != 0) {
                iputs(pyc_output, indent + 1, "[Locals+Kinds]\n");
                output_object(codeObj->localKinds().cast<PycObject>(), mod,
                    indent + 2, flags, pyc_output);
            }
            ...
            if (mod->verCompare(1, 5) >= 0 &&
                ...
                output_object(codeObj->lnTable().cast<PycObject>(), mod,
                    indent + 2, flags, pyc_output);
            }
        }
        break;
    ...
    case PycObject::TYPE_DICT:
        {
            ...
            for (const auto& val : obj.cast<PycDict>()->values()) {
                output_object(
                    std::get<0>(val), mod, indent + 1, flags, pyc_output);
                output_object(
                    std::get<1>(val), mod, indent + 2, flags, pyc_output);
            }
            iputs(pyc_output, indent, "}\n");
        }
        break;
    ...
    case PycObject::TYPE_COMPLEX:
        iprintf(pyc_output, indent, "(%s+%sj)\n",
            obj.cast<PycComplex>()->value(), obj.cast<PycComplex>()->imag());
        break;
    ...
    for (int arg = 1; arg < argc; ++arg) {
        if (strcmp(argv[arg], "-o") == 0) {
            ...
            fputs("  --pycode-extra Show extra fields in PyCode object dumps\n",
                stderr);
            fputs(
                "  --show-caches  Don't suprress CACHE instructions in Python 3.11+ disassembly\n",
                stderr);
            fputs(
