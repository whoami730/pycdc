...
#define DECLARE_PYTHON(maj, min) extern int python_##maj##_##min##_map(int);
...
#undef OPCODE
    ...
    static_assert(
        sizeof(opcode_names) / sizeof(opcode_names[0]) == PYC_LAST_OPCODE,
...
void print_const(std::ostream& pyc_output, PycRef<PycObject> obj,
    PycModule* mod, const char* parent_f_string_quote)
{
    if (obj == NULL) {
        pyc_output << "<NULL>";
        return;
    ...
    case PycObject::TYPE_SHORT_ASCII_INTERNED:
        obj.cast<PycString>()->print(
            pyc_output, mod, false, parent_f_string_quote);
        break;
    ...
    case PycObject::TYPE_LONG:
        formatted_print(
            pyc_output, "%s", obj.cast<PycLong>()->repr(mod).c_str());
        break;
    ...
    case PycObject::TYPE_BINARY_COMPLEX:
        formatted_print(pyc_output, "(%g+%gj)",
            obj.cast<PycCComplex>()->value(), obj.cast<PycCComplex>()->imag());
        break;
...
void bc_next(
    PycBuffer& source, PycModule* mod, int& opcode, int& operand, int& pos)
{
    opcode =
        Pyc::ByteToOpcode(mod->majorVer(), mod->minorVer(), source.getByte());
    if (mod->verCompare(3, 6) >= 0) {
        operand = source.getByte();
        pos += 2;
        if (opcode == Pyc::EXTENDED_ARG_A) {
            opcode = Pyc::ByteToOpcode(
                mod->majorVer(), mod->minorVer(), source.getByte());
...
{
    static const char* cmp_strings[] = { "<", "<=", "==", "!=", ">", ">=", "in",
        "not in", "is", "is not", "<EXCEPTION MATCH>", "<BAD>" };
    static const size_t cmp_strings_len =
        sizeof(cmp_strings) / sizeof(cmp_strings[0]);
    ...
    static const size_t binop_strings_len =
        sizeof(binop_strings) / sizeof(binop_strings[0]);
    ...
    static const size_t intrinsic1_names_len =
        sizeof(intrinsic1_names) / sizeof(intrinsic1_names[0]);
    ...
    static const size_t intrinsic2_names_len =
        sizeof(intrinsic2_names) / sizeof(intrinsic2_names[0]);
    ...
    static const size_t format_value_names_len =
        sizeof(format_value_names) / sizeof(format_value_names[0]);
    ...
    while (!source.atEof()) {
        ...
        for (int i = 0; i < indent; i++)
            pyc_output << "    ";
        formatted_print(
            pyc_output, "%-7d %-30s  ", start_pos, Pyc::OpcodeName(opcode));
        ...
        if (opcode >= Pyc::PYC_HAVE_ARG) {
            ...
            case Pyc::LOAD_GLOBAL_A:
                try {
                    ...
                    if (mod->verCompare(3, 11) >= 0) {
                        if (operand & 1)
                            formatted_print(pyc_output, "%d: NULL + %s",
                                operand, code->getName(operand >> 1)->value());
                        else
                            formatted_print(pyc_output, "%d: %s", operand,
                                code->getName(operand >> 1)->value());
                    }
                    else {
                        formatted_print(pyc_output, "%d: %s", operand,
                            code->getName(operand)->value());
                    }
                } catch (const std::out_of_range&) {
                    formatted_print(pyc_output, "%d <INVALID>", operand);
                }
            ...
            case Pyc::LOAD_FROM_DICT_OR_GLOBALS_A:
                try {
                    auto arg = operand;
                    if (opcode == Pyc::LOAD_ATTR_A &&
                        mod->verCompare(3, 12) >= 0)
                        arg >>= 1;
                    formatted_print(pyc_output, "%d: %s", operand,
                        code->getName(arg)->value());
                } catch (const std::out_of_range&) {
                    formatted_print(pyc_output, "%d <INVALID>", operand);
                }
                break;
            ...
            case Pyc::LOAD_FROM_DICT_OR_DEREF_A:
                try {
                    formatted_print(pyc_output, "%d: %s", operand,
                        code->getCellVar(mod, operand)->value());
                } catch (const std::out_of_range&) {
                    formatted_print(pyc_output, "%d <INVALID>", operand);
                }
                break;
            ...
            case Pyc::INSTRUMENTED_POP_JUMP_IF_TRUE_A:
                {
                    ...
                    formatted_print(
                        pyc_output, "%d (to %d)", operand, pos + offs);
                }
                break;
            ...
            case Pyc::INSTRUMENTED_JUMP_BACKWARD_A:
                {
                    ...
                    int offs = static_cast<int>(
                        operand * sizeof(uint16_t));   // BPO-27129
                    formatted_print(
                        pyc_output, "%d (to %d)", operand, pos - offs);
                }
                break;
            ...
            case Pyc::COMPARE_OP_A:
                {
                    ...
                    if (static_cast<size_t>(arg) < cmp_strings_len)
                        formatted_print(
                            pyc_output, "%d (%s)", operand, cmp_strings[arg]);
                    else
            ...
            case Pyc::BINARY_OP_A:
                if (static_cast<size_t>(operand) < binop_strings_len)
                    formatted_print(
                        pyc_output, "%d (%s)", operand, binop_strings[operand]);
                else
                    formatted_print(pyc_output, "%d (UNKNOWN)", operand);
                break;
            case Pyc::IS_OP_A:
            ...
            case Pyc::SET_FUNCTION_ATTRIBUTE_A:
                ...
                case 0x01:
                    formatted_print(
                        pyc_output, "%d (MAKE_FUNCTION_DEFAULTS)", operand);
                    break;
                case 0x02:
                    formatted_print(
                        pyc_output, "%d (MAKE_FUNCTION_KWDEFAULTS)", operand);
                    break;
                case 0x04:
                    formatted_print(
                        pyc_output, "%d (MAKE_FUNCTION_ANNOTATIONS)", operand);
                    break;
                case 0x08:
                    formatted_print(
                        pyc_output, "%d (MAKE_FUNCTION_CLOSURE)", operand);
                    break;
                default:
                    formatted_print(pyc_output, "%d (UNKNOWN)", operand);
                    break;
...
void bc_exceptiontable(
    std::ostream& pyc_output, PycRef<PycCode> code, int indent)
{
