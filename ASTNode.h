...
public:
    ASTClass(
        PycRef<ASTNode> code, PycRef<ASTNode> bases, PycRef<ASTNode> name) :
...
public:
    ...
    ASTTuple(value_t values) :
        ASTNode(NODE_TUPLE), m_values(std::move(values)), m_requireParens(true)
    {
    }
...
public:
    ...
    ASTConstMap(PycRef<ASTNode> keys, const values_t& values) :
        ASTNode(NODE_CONST_MAP), m_keys(std::move(keys)),
...
public:
    ASTIterBlock(
        ASTBlock::BlkType blktype, int start, int end, PycRef<ASTNode> iter) :
